...
"""
function discretize_SIMPLEC_PressureCorrection end

function discretize_SIMPLEC_PressureCorrection(
    AU::Union{
    ...
    for i in 1:mesh.l1
        if phi.onoff[i]
            ...
            if (i != 1) && (mesh.l1 != 1) && (phi.onoff[i-1])
                ...
                D1 = mesh.vol[i]/AUsum[phi.gIndex[i]]
                D2 = mesh.vol[i-1]/AUsum[phi.gIndex[i-1]]
        ...
        for j in 1:mesh.m1
            if phi.onoff[i,j]
                ...
                if (i != 1) && (mesh.l1 != 1) && (phi.onoff[i-1,j])
                    ...
                    D1 = mesh.vol[i,j]/AUsum[phi.gIndex[i,j]]
                    D2 = mesh.vol[i-1,j]/AUsum[phi.gIndex[i-1,j]]
                    Dw = general_interpolation(
                        mesh.dx[i], mesh.dx[i-1], D1, D2;
                        interpolation = interpolation
                    )
                ...
                if (j != 1) && (mesh.m1 != 1) && (phi.onoff[i,j-1])
                    ...
                    D1 = mesh.vol[i,j]/AVsum[phi.gIndex[i,j]]
                    D2 = mesh.vol[i,j-1]/AVsum[phi.gIndex[i,j-1]]
            ...
            for k in 1:mesh.n1
                if phi.onoff[i,j,k]
                    ...
                    if (i != 1) && (mesh.l1 != 1) && (phi.onoff[i-1,j,k])
                        ...
                        D1 = mesh.vol[i,j,k]/AUsum[phi.gIndex[i,j,k]]
                        D2 = mesh.vol[i-1,j,k]/AUsum[phi.gIndex[i-1,j,k]]
                        Dw = general_interpolation(
                            mesh.dx[i], mesh.dx[i-1], D1, D2;
                            interpolation = interpolation
                        )
                    ...
                    if (j != 1) && (mesh.m1 != 1) && (phi.onoff[i,j-1,k])
                        ...
                        D1 = mesh.vol[i,j,k]/AVsum[phi.gIndex[i,j,k]]
                        D2 = mesh.vol[i,j-1,k]/AVsum[phi.gIndex[i,j-1,k]]
                        Ds = general_interpolation(
                            mesh.dy[j], mesh.dy[j-1], D1, D2;
                            interpolation = interpolation
                        )
                    ...
                    if (k != 1) && (mesh.n1 != 1) && (phi.onoff[i,j,k-1])
                        ...
                        D1 = mesh.vol[i,j,k]/AWsum[phi.gIndex[i,j,k]]
                        D2 = mesh.vol[i,j,k-1]/AWsum[phi.gIndex[i,j,k-1]]
