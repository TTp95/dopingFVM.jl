...
"""
function SIMPLER_pseudovelocity end

function SIMPLER_pseudovelocity(
        ...
        for j in 1:mesh.m1
            if (velocity.u.onoff[i,j] == 1.0)
                ...
                if (AU[id,id] != 0.0) && (AV[id,id] != 0.0)
                    ...
                    pseudo_u[i,j] = (bU[id] - sum(@views (AU[id,:]) .* u))/(ac_AU)
                    pseudo_v[i,j] = (bV[id] - sum(@views (AV[id,:]) .* v))/(ac_AV)
