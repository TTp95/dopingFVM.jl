...
"""
function PISO_correction! end

# function PISO_correction!(
    ...
    for i in 2:mesh.l1
        for j in 1:mesh.m1
            if velocity.u.onoff[i,j] && velocity.u.onoff[i-1,j]
                D1 = mesh.vol[i,j]/AU[velocity.u.gIndex[i,j],velocity.u.gIndex[i,j]]
                D2 = mesh.vol[i-1,j]/AU[velocity.u.gIndex[i-1,j],velocity.u.gIndex[i-1,j]]
    ...
    for i in 1:mesh.l1
        for j in 2:mesh.m1
            if velocity.v.onoff[i,j] && velocity.v.onoff[i,j-1]
                D1 = mesh.vol[i,j]/AV[velocity.v.gIndex[i,j],velocity.v.gIndex[i,j]]
                D2 = mesh.vol[i,j-1]/AV[velocity.v.gIndex[i,j-1],velocity.v.gIndex[i,j-1]]
